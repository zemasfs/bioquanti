...
// === Pong with 80s beeps (ASCII-only) ===
// - Paddle/wall: short square beeps
// - Miss (game over): lower sawtooth buzz
// - No "modalidade" slider

// Simple sound module using Web Audio (no external files)
const Sound = (function () {
  ...
  function beep(freq, duration, type, peakGain) {
    const c = ensureCtx();
    ...
    g.gain.linearRampToValueAtTime(peakGain || 0.03, t0 + 0.005);
    g.gain.exponentialRampToValueAtTime(0.0001, t0 + (duration || 0.08));
    osc.connect(g).connect(c.destination);
    osc.start(t0);
    osc.stop(t0 + (duration || 0.08) + 0.02);
  }
  return {
    resume: () => ensureCtx().resume(),
    paddle: () => beep(880, 0.06, "square", 0.03),
    wall:   () => beep(660, 0.06, "square", 0.025),
    miss:   () => beep(220, 0.28, "sawtooth", 0.05)
  };
})();
...
function criarPongInterativo() {
  new p5((sketch) => {
    let ball, paddle, playing = true, score = 0;
    let streak = 0;
    ...
    sketch.draw = function () {
      ...
      if (!playing) {
        ...
        sketch.textSize(18);
        sketch.text("Final Score: " + score, sketch.width / 2, sketch.height / 2 + 20);
        return;
      }
      ...
      const boost = 1 + streak * 0.03;
      const speed = base * boost;
      const dir = Math.atan2(ball.vy, ball.vx);
      ball.vx = speed * Math.cos(dir);
      ball.vy = speed * Math.sin(dir);

      // HUD
      sketch.fill(80);
      sketch.textSize(16);
      sketch.textAlign(sketch.LEFT, sketch.TOP);
      sketch.text("Score: " + score, 10, 10);

      // ball
      sketch.fill(50, 100, 200);
      sketch.circle(ball.x, ball.y, ball.r * 2);
      ball.x += ball.vx;
      ball.y += ball.vy;

      // walls
      if (ball.x < ball.r || ball.x > sketch.width - ball.r) {
        ball.vx *= -1;
        streak++;
        Sound.wall();
      }
      ...
      sketch.rect(paddle.x, paddle.y, paddle.w, paddle.h);
      paddle.x = sketch.constrain(paddle.x, 0, sketch.width - paddle.w);
      ...
      // miss
      if (ball.y - ball.r > sketch.height) {
        playing = false;
        Sound.miss();
      }

      // streak decay every ~2s
      if (streak > 0 && sketch.frameCount % 120 === 0) {
        streak--;
      }
    };
    ...
    sketch.touchMoved = function () {
      paddle.x = sketch.constrain(sketch.mouseX - paddle.w / 2, 0, sketch.width - paddle.w);
      return false;
    };
...
if (typeof editor !== "undefined" && document.getElementById("grafico")) {
  ...
  graficoDiv.innerHTML = [
    ...
    '  <button id="btnStart">Start Game</button><br><br>',
    '  <label>Speed: <span id="warpValor">Warp 1.5</span></label><br>',
...
window.salvarProjetoSomenteGrafico = function () {
  ...
  var codigoSketch = criarPongInterativo.toString();

  // Inline Sound module (mesmo do app, ASCII-only)
  var codigoSom = `
const Sound = (function () {
  ...
  function beep(freq, duration, type, peakGain) {
    ...
    osc.frequency.setValueAtTime(freq || 440, t0);
    g.gain.setValueAtTime(0.0001, t0);
    g.gain.linearRampToValueAtTime(peakGain || 0.06, t0 + 0.005);
    g.gain.linearRampToValueAtTime((peakGain || 0.06) * 0.6, t0 + (duration || 0.09) * 0.6);
    g.gain.linearRampToValueAtTime(0.0001, t0 + (duration || 0.09));
    osc.connect(g).connect(master || c.destination);
    osc.start(t0);
    osc.stop(t0 + (duration || 0.09) + 0.02);
  }
  return {
    resume: () => ensureCtx().resume && ensureCtx().resume(),
    paddle: () => beep(900, 0.08, "square", 0.10),
    wall:   () => beep(700, 0.08, "square", 0.08),
    miss:   () => beep(180, 0.35, "sawtooth", 0.09)
  };
})();`;
...
<body>
  ...
  <script>
    document.getElementById("btnStart").addEventListener("click", function () {
      if (window.Sound && Sound.resume) { try { Sound.resume(); } catch(e){} }
      document.getElementById("jogoArea").innerHTML = "";
      criarPongInterativo();
    });
    document.getElementById("warpSlider").addEventListener("input", function () {
...
function salvarAppCompleto() {
  ...
  // Script que será injetado no HTML salvo para redesenhar o gráfico
  const scriptPlotly = `
    <script>
