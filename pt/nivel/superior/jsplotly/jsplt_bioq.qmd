...
```{r,eval=FALSE}
1. Experimente trocar a sequência para um conhecido;
...
1. Abra o app num navegador e clique em "add". Será gerado um conjunto de 7 barras coloridas, representando as cores obtidas em tubos de ensaio para 7 reações distintas para caracterização proteica:  ninhidrina, biureto, Pauly, xantoproteica, Sakagushi, Millon, e enxofre (Cys);
...
3. Após a seleção nos 2 menus, clique em "Conferir". Se tiver acertado, um "Placar" logo abaixo irá computar número de acertos/número de amostras "jogadas";

4. Se errar, tente de novo ou busque a explicação do padrão de cores em "Explicar";

5) "Modo difícil" esconde o nome das reações, embora mantendo sua sequência, e "Embaralhar" altera a ordem dos tubos;

6) Para uma nova amostra, clique em "Nova amostra";


**Opção B: descobrir o padrão de cores de uma reação para uma amostra selecionada:**

1. Selecione uma amostra em "Gerar", e clique em "Nova amostra". Será gerado um conjunto de tubos com padrão de reações esperado para a amostra.
