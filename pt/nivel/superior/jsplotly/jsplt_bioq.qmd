---
title: "Simulações Interativas Para Biofísico-química com JSPlotly"
format: html
toc: true
toc-depth: 3
...
|   Para ilustrar o potencial de uso do *JSPlotly* para Bioquímica e áreas correlatas, seguem alguns exemplos de **simulações, análise de dados, e mesmo aplicativos**, e cujos conteúdos são frequentemente encontrados em livros-texto e fontes técnico-científicas. Para um melhor aproveitamento de cada tema, experimente seguir as sugestões dos códigos para cada objeto. 
\
...
#3# Diagnóstico de Inibição Enzimática (Michaelis-Mentem e Lineweaver-Burk)
...
### CarboId - Experimentação simulada para caracterização de aminoácidos e cadeias  
...
### ZeMol: um visualizador 3D para modelos moleculares
...
### ProtId - Experimentação simulada para caracterização de carboidratos
...
### CinDroga - Experimentação simulada para prospecção de composto líder como inibidor enzimático


|   O *JSPlotly* pode operar como um aplicativo completo de simulação em situações variadas. O exemplo que se apresenta refere-se a seu uso como um painel (*dashboard*) para simular o potencial de uma molécula fictícia em triagem visando a seleção de composto líder (*lead compound*) como inibidor a um alvo enzimático.
\

*Observação*: o *CinDroga* possui códigos internos para estilos e montagens do painel e que interferem no *bootstratp* do criador de páginas utilizado (pacote *Quarto* para *R*). Nesse sentido, é preferível baixá-lo como um arquivo comprimido e executá-lo isoladamente, clicando-se na imagem que segue.

[![](Eq/leadCompound.png)](Eq/jsp_leadCompound.html.zip){target="_blank"}





|   Há dois modos para estudo para uso do *CinDroga*: inserindo-se parâmetros cinéticos e termodinâmicos para observar os resultados expressos em gráficos (botão *parâmetros*), ou caracterizar os parâmetros a partir dos gráficos (botão *diagnóstico*). 
...
|   Seja qual for o modo experimentado, os parâmetros são:

* *Ki*, constante de equilíbrio de dissociação do inibidor;
...
|   Para usar o *CinDroga* para caracterizar um candidato a composto líder basta carregar o script (*add*). Serão apresentados 1) os descritores do composto, 2) quatro gráficos com simulação de resultados experimentais de cinética (Michaelis-Mentem, Lineweaver-Burk, e curva dose-resposta) e termodinâmica (van't Hoff). A partir da visualização é possível se determinar:
...
|   Dessa forma, pode-se utilizar o *CinDroga* para uma simulação significativa à triagem de um inibidor como *lead compound* à uma enzima-alvo.
...
2. Experimente o modo "parâmetros" com valor de nH diferente de 1, para avaliar o comportamento da curva dose-reposta sob efeitos de cooperatividade;
3. Experimente o modo "parâmetros" com valor para capacidade calorífica diferente de zero, para avaliar a possibilidade de efeitos secundários na ligação em função da temperatura, como transição conformacional da enzima;
```
\
...
- Utilizar um *apontador de laser* para localização transiente do que se pretende assinalar;
