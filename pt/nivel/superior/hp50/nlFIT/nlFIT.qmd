---
title: "nlFIT - Ajuste não-linear e cinética enzimática"
format: html
toc: true
toc-depth: 3
...
|   Este programa permite efetuar um ajuste não linear por método de Gauss-Newton para uma equação introduzida pelo usuário e dados em listas. O programa foi adaptado do código do programa DATFIT (Coffin, 1995), e encontra-se depositado junto ao portal [hpcalc.org](https://www.hpcalc.org) neste [link](https://www.hpcalc.org/search.php?query=nlFIT). Possui a seguinte logística e diferenciais:
\
...
6. Utiliza as subrotinas FDER e PGALL do mesmo livro para cálculo da derivada simbólica da função, bem como para para eliminar variáveis previamente armazenadas, respectivamente (o programa original necessita de 3 subprogramas para rodar);
7. Plota os dados experimentais com rotina sem necessidade de ΣDAT (PIXON).
```
\

##  Arquivos

1. [Programa nlFIT e exemplo](nlFIT.zip)
2. [Código](nlFIT.pdf)
\
...
|   Junto ao arquivo comprimido segue um exemplo em cinética enzimática para ilustrar seu uso. Os dados referem-se ao dataset *L.minor* de captação de nitrogênio por macrófitas, tal como apresentado no pacote `nlstools` do programa de computação estatística [R](https://cran.r-project.org/).
...
|   Os resultados do ajuste não linear obtidos com o programa *nlFIT* são quase idênticos aos obtidos pelo software estatístico `R`, conforme tabela abaixo. Não obstante, *nlFIT* requereu apenas 3 iterações, contra 7 do *R*.
