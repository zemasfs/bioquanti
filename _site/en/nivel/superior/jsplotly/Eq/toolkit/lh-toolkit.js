...
/* ===== Lead Hunt — ensino de Ki/IC50/van’t Hoff (ΔCp curvo + header limpo) ===== */
(function(){
  // ---------- utils ----------
...
function v_mista(S, Vmax, Km, I, Ki_c, Ki_u){
  ...
  // molécula aleatória + Lipinski básicos
  ...
  function startLeadHunt(targetId){
    ...
    // header (remove α-replot aqui)
    var header=make("div",{style:{display:"flex",gap:"8px",flexWrap:"wrap",alignItems:"center",marginBottom:"6px"}});
    var btnReset=make("button",{title:"novo experimento (aleatório)"},"reset");
    var btnHTML =make("button",{title:"exportar página HTML com os gráficos"},"html");
    var btnDiag =make("button",{title:"diagnóstico (respostas)"},"diagnóstico");
    var btnParam=make("button",{title:"definir parâmetros e gerar"},"parâmetros");
    ...
    // grid de gráficos: MM/LB em cima; IC50 e van’t Hoff lado a lado
...
// painel tabela + decidir (fix: sem usar variável 'panel')
...
root.appendChild(panelRow);
    ...
    // painel Ki/aluno (aqui mantém o botão de α-replot, que é útil)
...
if (state.mech === "competitiva")      vfun = v_comp;
  ...
  vfun = function(S,Vmax,Km,I,Ki_dummy){
    ...
    function renderMM(clearOnly){
      var layout={ title:"Michaelis–Menten",
...
var yy = [a*xx[0] + b, a*xx[1] + b];
    ...
    function renderTable(){
      ...
      tableBox.innerHTML =
        '<div style="font-weight:bold; margin-bottom:6px;">Parâmetros aparentes</div>'+
...
if (state.mech === "competitiva")      vfun = v_comp;
  ...
  Plotly.newPlot(plotI, [
    ...
    {x:xs,    y:ys,            mode:"markers", name:"pontos (traços)", marker:{size:7}}
  ], {
    title: "Curva Dose-Resposta",
    xaxis:{ title:"[I] (log)", type:"log" },
    yaxis:{ title:"% inibição", range:[0,100] },
    ...
    function renderVanthoff(){
      ...
      // R² do polinômio (sobre os pontos)
      ...
      Plotly.newPlot(plotVH, [
        ...
        {x:gridX, y:gridY,mode:"lines",   name:"modelo (ΔCp)", line:{dash:"dot"}},
        {x:xx,    y:yy,   mode:"lines",   name:"ajuste linear"},
        {x:polyX, y:polyY,mode:"lines",   name:"ajuste polinomial (2º)", line:{dash:"dash"}}
      ], {
        title:"van’t Hoff",
        xaxis:{title:"1/T (K⁻¹)"},
        yaxis:{title:"ln K"},
        ...
        annotations:[{
          ...
          text:"Linear: m="+st.a.toFixed(3)+", b="+st.b.toFixed(3)+", R²="+st.r2.toFixed(3)+
               "<br>Polinômio(2º): R²="+r2p.toFixed(3),
          showarrow:false,font:{size:12}
        }]
      }, {displaylogo:false});
    }

    // ===== Ki (I vs valor do aluno) — livre =====
...
for (var i = 0; i < state.Is.length; i++) {
  ...
  btnAlpha.onclick = function(){ plotAlphaReplot(); };
    ...
    function plotKiStudent(){
      ...
      if (ys.length < 2){
        if (xs.length === 1){
          ...
          if (statsEl) statsEl.textContent = "Insira ≥ 2 pontos para ajuste linear.";
        } else {
          Plotly.purge(plotKI);
          if (statsEl) statsEl.textContent = "Forneça pelo menos 2 valores.";
        }
        return;
      }
    ...
    function plotAlphaReplot(){
      ...
      Plotly.newPlot(plotKI, [
        {x: xs, y: ys, mode:"markers", name:"pontos (α−1)"},
        {x: xx, y: yy, mode:"lines", name:"ajuste (origem)"}
      ], {
        title:"α−1 vs [I] — Ki = 1/slope",
        xaxis:{ title:"[I]", range:[xMin, xMax] },
        yaxis:{ title:"α − 1" },
        margin:{ l:50, r:10, t:40, b:45 },
        shapes: axisLineAtI0(),
  ...
  for (var i=0; i<list.length; i++){
    ...
    function openModal(html){
      ...
      box.innerHTML = '<div style="text-align:right;"><button id="xClose" title="fechar">×</button></div>'+html;
...
function buildDiagHTML(){
  ...
  var L=[];
  L.push('<div style="font-weight:700;margin-bottom:6px">Diagnóstico</div>');
  ...
  L.push('');
  L.push("van't Hoff:");
  L.push('  ΔH = '+dH_kcal.toFixed(3)+' kcal/mol');
  L.push('  ΔS = '+dS_kcalK.toExponential(3)+' kcal/mol·K');
  ...
  L.push('');
  L.push('faixas típicas: MW≤500, HBD≤5, HBA≤10, cLogP≤5; LE ≥ 0.3; LLE ≥ 5');
  L.push('</div>');
  return L.join('');
}
...
function openDiag(){ openModal(buildDiagHTML())
  ...
  openModal(L.join(String.fromCharCode(10)));
    ...
    function openParamModal(){
      var html =
        '<div style="font-weight:700;margin-bottom:8px">Definir parâmetros e gerar</div>'+
        '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;font:12px monospace">'+
          ...
          '<label>ΔH (J/mol) <input id="pdH" type="number" step="100" value="'+state.dH.toFixed(0)+'"></label>'+
          '<label>ΔS (J/mol·K) <input id="pdS" type="number" step="1" value="'+state.dS.toFixed(1)+'"></label>'+
          '<label>ΔCp (J/mol·K) <input id="pdCp" type="number" step="10" value="'+state.dCp.toFixed(0)+'"></label>'+
          '<label>mecanismo '+
            '<select id="pMech">'+
      ...
      // esconder/mostrar Ki_u conforme mecanismo (mova para cá)
...
toggleKiu();
    ...
    function initNewExp(generateCurvesToo){
      ...
      state.nH = 0.6 + 1.8*state.rng();               // 0.6–2.4
    ...
    //  seedSpan.textContent="seed="+state.seed;
      molCard.innerHTML='<div style="font-weight:600;margin-bottom:4px;">Molécula fictícia</div>'+
                        '<div style="font-family:monospace">Fórmula: '+state.mol.formula+
      ...
      Plotly.newPlot(plotMM, [], {title:"Michaelis–Menten", xaxis:{title:"[S]"}, yaxis:{title:"v"},
                      margin:{l:50,r:10,t:40,b:45}, showlegend:true, legend:{orientation:"h"}}, {displaylogo:false});
      Plotly.newPlot(plotLB, [], {title:"Lineweaver–Burk (1/v vs 1/[S])", xaxis:{title:"1/[S]"}, yaxis:{title:"1/v"},
                      margin:{l:50,r:10,t:40,b:45}, showlegend:true, legend:{orientation:"h"}}, {displaylogo:false});
      Plotly.newPlot(plotI,  [], {title:"% inibição vs [I] (log)", xaxis:{title:"[I] (log)", type:"log"}, yaxis:{title:"% inibição"},
                      margin:{l:50,r:10,t:40,b:45}, showlegend:true, legend:{orientation:"h"}}, {displaylogo:false});
      Plotly.newPlot(plotVH, [], {title:"van’t Hoff — ln K vs 1/T", xaxis:{title:"1/T (K⁻¹)"}, yaxis:{title:"ln K"},
                      margin:{l:50,r:10,t:40,b:45}, showlegend:true, legend:{orientation:"h"}}, {displaylogo:false});
      Plotly.purge(plotKI);
    ...
    function estimateKiAndCheck(userType, userKi){
      ...
      if (Is.length<2) return {ok:false, msg:"precisa de >=2 níveis de inibidor"};
      var Ki_OLS=NaN;
  ...
  decideBox.innerHTML="";
  var title=make("div",{style:{fontWeight:"bold",marginBottom:"6px"}}, "DIAGNÓSTICO PROPOSTO");
  ...
  var sel  =make("select",{},[
    ...
    make("option",{value:"mista"},"mista") // <<< vírgula adicionada na linha acima
  ]);
  ...
  btnCheck.onclick=function(){
    ...
    if (!res){ out.textContent="resultado indisponível"; return; }
    var msgs=[]; msgs.push(res.typeOK? "Tipo OK" : "Tipo incorreto");
    msgs.push(res.kiOK? "Ki (LB) OK (≤5%)" : "Ki (LB) fora da margem");
    msgs.push(res.msg);
    out.textContent=(res.typeOK && res.kiOK)? ("Parabéns! "+msgs.join(" | ")) : ("Tente novamente. "+msgs.join(" | "));
  };
}
...
btnDiag.onclick = function(ev){
  ...
  closeAllModals();    // reforço extra
  openDiag();          // sua função atual de diagnóstico (a versão “boa”)
};
