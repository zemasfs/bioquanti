...
### JSPlotly for Arduino and conductivity data acquisition (EM13CNT201, EM13CNT308, M13CNT301, M13CNT302) {.unnumbered}
...
1. For *Web Serial* connection, you need to install *Python* on your computer (Windows or Linux) and set up a local server. For Windows, download and install the official file for [Python](https://www.python.org/downloads/windows/), remembering to click on *Add Python to PATH*, and type the commands in CMD inside the standalone application folder. Here are the instructions for Linux:

- 1. Connect the Arduino and components (TDS probe, container with water and salt);
- 2. Copy, paste, and run the manufacturer's *script* for TDS in the connected Arduino IDE;
- 3. Close the Arduino IDE;
- 4. Open a Terminal in the standalone application folder of *JSPlotly* for Arduino (*condutiv_JSPlotly.html*);
- 5. Type: *python3 -m http.server*
- 6. Open the standalone html of the circuit in Chrome (never in Firefox);
